...
- `internal_automation_site.py`: 사이트 실행 파일
- `requirements.txt`: 실행에 필요한 Python 패키지 목록
- `README.md`: 실행 방법과 기능 설명
- `assets/fonts/`: 선택 폰트 폴더입니다. 큰 폰트 파일은 업로드하지 않아도 되며, 없으면 앱이 기본 한글 PDF 폰트로 대체합니다.
...
```bash
streamlit run /Users/choeinhye/Desktop/internal_automation_site.py
```
...
새 메뉴를 추가하려면 `internal_automation_site.py`의 `NAV_ITEMS`에 항목을 추가하고, `render_page()`에서 해당 화면 렌더링 함수를 연결하면 됩니다.
