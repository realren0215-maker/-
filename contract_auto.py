...
def fill_contract_excel(inputs: ContractInputs) -> bytes:
    ...
    if not template_path.exists():
        raise FileNotFoundError(f"계약서 템플릿이 없습니다: {template_path}")
    ...
    set_cell(sheet, "F11", "")
    set_cell(sheet, "C52", "")
    set_cell(sheet, "K52", f"套餐：{inputs.ad_product}")
    if inputs.memo.strip():
        set_cell(sheet, "C54", f"{sheet['C54'].value or ''}\n补充备注：{inputs.memo.strip()}")
    add_image_to_sheet(sheet, inputs.signature_bytes, "D58", 160, 58)
    add_image_to_sheet(sheet, inputs.stamp_bytes, "F57", 110, 78)
...
def render_app(embedded: bool = False) -> None:
    ...
    with right:
        ...
        ad_product = st.text_input("广告商品 / 套餐", value="美食商户通套餐（进阶版）" if contract_type == "1年" else "美食商户通套餐（基础版）")
