...
def set_cell_value(ws, cell: str, value: object) -> None:
    ws[cell] = value
    ws[cell].alignment = ws[cell].alignment.copy(wrap_text=True, vertical="center")


def make_contract_file_name(inputs: ContractInputs, ext: str) -> str:
    return f"계약서_{safe_filename(inputs.store_name)}_{inputs.contract_type}.{ext}"
...
def fill_contract_workbook(inputs: ContractInputs) -> bytes:
    ...
    duration_text = "365  】天" if inputs.contract_type == "1년" else "3】个月"
    package_text = "美食商户通套餐（进阶版）" if inputs.contract_type == "1년" else "美食商户通套餐（基础版）"
    ...
    set_cell_value(ws, "C11", f"合计：人民币￥ {inputs.cpc_amount:,.2f}")
    set_cell_value(ws, "F11", f"大写金额：人民币 {inputs.cpc_amount:,.0f} 元整")
    set_cell_value(ws, "C12", f"☐ 推广计划开始时间 {start_text}")
    set_cell_value(ws, "H12", f"日预算人民币￥ {inputs.daily_budget:,.0f}")
    set_cell_value(ws, "L19", f"自开通之日起【{duration_text}")
    set_cell_value(ws, "C52", f"合计：人民币￥{inputs.total_amount:,.2f}                        大写金额：人民币 {inputs.total_amount:,.0f} 元整")
    set_cell_value(ws, "K52", f"套餐：{package_text}")
    ...
    if inputs.stamp_bytes:
        with NamedTemporaryFile(suffix=".png", delete=False) as tmp:
            tmp.write(inputs.stamp_bytes)
            tmp_path = tmp.name
        stamp = OpenpyxlImage(tmp_path)
        stamp.width = 130
        stamp.height = 72
        ws.add_image(stamp, "D57")
...
def contract_pdf_bytes(inputs: ContractInputs) -> bytes:
    ...
    rows = [
        ...
        ["CPC 금액", f"{inputs.cpc_amount:,.2f}위안"],
        ["일 예산", f"{inputs.daily_budget:,.0f}위안"],
        ["본 주문 총액", f"{inputs.total_amount:,.2f}위안"],
    ]
    story = [
        Paragraph("계약서", title),
        Spacer(1, 4 * mm),
...
def run_cpc_report_app() -> None:
    ...
    with left:
        with st.container(border=True):
            ...
            uploaded_logo = None
            if use_logo:
                uploaded_logo = st.file_uploader("로고 이미지 선택", type=["png", "jpg", "jpeg"], help="업로드한 로고가 보고서 상단에 표시됩니다.")
...
def run_contract_app() -> None:
    ...
    with left:
        with st.container(border=True):
            ...
            contract_type = st.radio("계약기간", ["1년", "3개월"], horizontal=True)
            store_name = st.text_input("매장명", key="contract_store_name", placeholder="예: 태초갈비 홍대점")
            address_kr = st.text_area("주소", key="contract_address_kr", placeholder="한국어 주소 입력")
            suggested_address = auto_english_address(address_kr)
            address_en = st.text_area("영문 주소", value=suggested_address, help="자동 변환값을 확인 후 필요하면 직접 수정해주세요.")
            business_name_kr = st.text_input("사업자명(한글)", placeholder="예: 홍길동")
            passport_english_name = st.text_input("여권 영문명", placeholder="예: HONG GILDONG")
            passport_number = st.text_input("여권번호", placeholder="선택 입력")
            region = st.text_input("지역", placeholder="예: Seoul")
            store_code = st.text_input("매장코드", placeholder="예: 12345678")
            contract_start = st.date_input("계약/운영 시작일", value=date.today())

    with right:
        with st.container(border=True):
            st.subheader("금액 및 도장")
            cpc_amount = st.number_input("CPC 금액(위안)", min_value=0.0, value=5000.0 if contract_type == "1년" else 3000.0, step=100.0)
            daily_budget = st.number_input("일 예산(위안)", min_value=0.0, value=100.0, step=10.0)
            total_amount = st.number_input("본 주문 총액(위안)", min_value=0.0, value=8000.0 if contract_type == "1년" else 3000.0, step=100.0)
            memo = st.text_area("비고", key="contract_memo", placeholder="선택 입력")
            stamp_file = st.file_uploader("직인(도장) PNG 업로드", type=["png"], help="업로드 시 계약서 하단 지정 위치에 삽입됩니다.")
            generate_contract = st.button("계약서 생성", type="primary", use_container_width=True)
            st.markdown(
                """
                <div class="hint">
                계약서 템플릿은 앱 내부에 저장되어 있습니다. 향후 양식이 바뀌면 assets/templates 폴더의 템플릿 파일만 같은 이름으로 교체하면 됩니다.
                </div>
                """,
                unsafe_allow_html=True,
            )
    ...
    if not store_name.strip():
        st.error("매장명을 입력해주세요.")
        return
    if not address_kr.strip() and not address_en.strip():
        st.error("주소를 입력해주세요.")
        return
    if not passport_english_name.strip() and not business_name_kr.strip():
        st.error("사업자명 또는 여권 영문명을 입력해주세요.")
        return
    ...
    try:
        ...
        cols = st.columns(2)
        cols[0].download_button("계약서 Excel 다운로드", data=xlsx_bytes, file_name=make_contract_file_name(inputs, "xlsx"), mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
        cols[1].download_button("계약서 PDF 다운로드", data=pdf_bytes, file_name=make_contract_file_name(inputs, "pdf"), mime="application/pdf", use_container_width=True)
    except Exception as exc:
